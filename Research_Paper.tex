...
% ---------------------------------------------------------------
\begin{abstract}
We investigate adversarially exposed supervised AI text detection under a
held-out-generator evaluation protocol at consumer-GPU scale. A binary
...
families. All four ablation configurations achieve Relative Robustness
Degradation values between 1.1\% and 3.1\%, with held-out F1 ranging from
0.913 to 0.927---well within our proposed 5\% generalisation threshold. The ablation
...
Fast-DetectGPT is reproduced with AUROC 0.937 and extended with 4-bit NF4
...
destructive than paraphrase rewriting (gpt2-xl RRD: 33.40\%).
\end{abstract}
...
fine-tuning on adversarially diverse RAID data transfers to a distinct
...
it is a compact model deployable within the 8~GB VRAM constraint of
...
\smallskip
\noindent\textbf{RQ1:} Does a DistilBERT model trained on adversarially diverse RAID
...
DistilBERT study, while specific RAID attack subsets support the Fast-DetectGPT
extension.
...
seen RAID generators to withheld RAID generators under an adversarially diverse
...
trained on adversarially diverse samples while reserving entire generator
...
\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{figures/figure1_architecture.png}}
...
\begin{figure*}[t]
\centerline{\includegraphics[width=\textwidth]{figures/figure2_split_pipeline.png}}
...
families under RAID's adversarially diverse distribution.
...
within 8~GB VRAM in the main ablation run.
...
minimising CrossEntropyLoss over two-class softmax outputs. After softmax normalization, class prediction follows argmax over the two probability outputs, equivalent to a 0.5 threshold on the binary posterior probability for class 1. Gradients are clipped
...
The primary evaluation metric is positive-class binary F1 score, with the AI
class treated as positive. Because all reported DistilBERT splits are balanced,
this metric is directly interpretable alongside accuracy, precision, and recall.
...
\label{eq:rrd}
\end{equation}
Lower RRD indicates superior held-out-generator generalisation. Following an interpretive heuristic (no established significance
...
that generalisation has failed. This threshold is chosen descriptively to contextualise observed RRD values and does not reflect formal significance testing; see \ref{ssec:limits} for this limitation.
...
the full 8~GB dedicated capacity.
...
RTX~3050 8~GB GPU, AMD Ryzen 5600X CPU (6 cores / 12 threads), 32~GB DDR4 RAM,
and NVMe SSD storage for parquet and tensor-cache I/O. The software stack is
Python~3.10.14, PyTorch~2.2.0 + CUDA~12.1, Transformers~4.40.0,
Datasets~2.19.0, scikit-learn~1.4.2, accelerate~0.30.1, NumPy~1.26.4, and
pandas~2.2.2. All experiments use \texttt{SEED=42} with
...
\textbf{Tokeniser compatibility patch.} The official Fast-DetectGPT
...
The RAID adversarial evaluation of Fast-DetectGPT was run locally with
...
seed=42). We use 2,500 MC samples, bootstrap 95\% CIs with 1,000 iterations,
...
four configurations.
Held-out F1 ranges from 0.913 to 0.927 (Table~\ref{tab:ablation}), and RRD values span 1.1\% to
3.1\%---well below the 5\% interpretive threshold established in
\eqref{eq:rrd}. This answers RQ1 positively for the RAID generator split:
representations learned from adversarially diverse seen-generator data
...
The confusion matrices (Table~\ref{tab:conf_seen}) reveal a critical
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/figure6_efficiency_speedup.png}}
...
AMP-T --- AMP + large batch      & Peak VRAM (MB)    & 8,000 (OOM)       & \textbf{4,218.95} & $-$47.3\% VRAM\\
...
Figure~\ref{fig:raid_auroc} and Table~\ref{tab:raid} present Fast-DetectGPT's
AUROC on RAID adversarial subsets.

\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/figure7_raid_auroc_comparison.png}}
...
original reproduction baseline.
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/figure9_quantization_efficiency.png}}
...
hardware transition (T4 to RTX 3050), the VRAM chart demonstrates the main benefit of 
4-bit NF4 quantisation: all compression configurations fall safely below the RTX~3050's 
8~GiB dedicated VRAM limit (dashed orange line), preventing OOM crashes.}
\label{fig:quant}
\end{figure}
...
Homoglyph attacks are substantially more destructive than paraphrase
...
The \texttt{gpt2-xl} RRD of 33.40\% demonstrates that attack type---rather
...
confound due to the transition from the cloud-based Tesla~T4 to the local RTX~3050, 
...
failures to run comfortably within the 8~GB dedicated VRAM ceiling of consumer-grade hardware. 
...
\eqref{eq:rrd}. Held-out F1 values ranging from 0.913 to 0.927 demonstrate
strong performance on generator families never encountered during training.
Addressing the second part of RQ1, DistilBERT's held-out F1 remains strong
while Fast-DetectGPT degrades on RAID adversarial attack subsets (homoglyph:
...
study demonstrates that architectural choices modulate the \emph{degree} of
...
head extracts superior in-distribution signal at a marginal cost to held-out
...
(0.913--0.927) and Fast-DetectGPT's adversarial RAID AUROC (0.526--0.789)
...
\textbf{Attack-family granularity.} The primary DistilBERT evaluation is a
...
\textbf{Single-seed evaluation.} All DistilBERT ablation results
...
\textbf{Absence of a clean-only training baseline.}
Every DistilBERT
...
\textbf{Human-text overlap across splits.} The training pool and held-out
evaluation pool share 6,029 exact human-written text matches. These duplicates
arise because the RAID corpus records the same human reference texts across
multiple generator splits. Although the AI-text portion is strictly
non-overlapping, the shared human samples may inflate held-out
accuracy by reinforcing familiar human writing patterns (since every held-out
human reference text has been seen during training). Future preprocessing
should deduplicate human reference texts across all splits to eliminate this
source of metric inflation.
...
DistilBERT binary classifier fine-tuned on adversarially diverse RAID data
achieves a Relative Robustness Degradation of 1.1--3.1\% on withheld generator
families. The model secures held-out F1 between 0.913 and 0.927, providing
positive evidence that a DistilBERT-based detector can transfer across RAID
generator families. The ablation study demonstrates that no single
...
under 16~minutes, achieves a favourable robustness margin over a zero-shot
probabilistic detector under adversarial attack conditions.
...
The experiments use public RAID, DetectRL, and Fast-DetectGPT resources.
