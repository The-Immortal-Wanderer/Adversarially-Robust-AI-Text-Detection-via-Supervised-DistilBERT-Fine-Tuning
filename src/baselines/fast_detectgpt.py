"""Fast DetectGPT baseline for DetectRL.

This implementation follows the user's requested simplification: score each text
with GPT-2 XL log p(x), optimize a threshold on the validation split, then
report classification metrics on the seen test and unseen test splits.
"""
...
DEFAULT_OUTPUT_PATH = RESULTS_DIR / "baseline_fastdetectgpt.json"
...
class FastDetectGPTConfig:
    """Configuration for the Fast DetectGPT baseline."""
...
def _get_device() -> torch.device:
    if not torch.cuda.is_available():
        raise RuntimeError("Fast DetectGPT requires CUDA because the requested model is loaded with device_map='cuda'.")
...
def run_fast_detectgpt(
    *,
    ...
    """Run the Fast DetectGPT baseline and save results to disk."""

    device = _get_device()
    print(f"Using device: {device}")

    _train_loader, val_loader, seen_test_loader = get_dataloaders()
    ...
    print(f"Saved Fast DetectGPT results to {output_path}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Fast DetectGPT baseline on DetectRL.")
